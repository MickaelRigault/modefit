...
class MCMC(BaseObject):
        ...
        **kwargs could be any `properties` entry:
        ...
        if not self.is_setup():
            raise AttributeError("At least one of the following proprety" +
                                 "has not been setup: nrun, nwalkers, guess," +
                                 " guess_err")
        self.reset(reset_property=False)

        # -- run the mcmc
        self._derived_properties["sampler"] = emcee.EnsembleSampler(
            self.nwalkers, self.nparam, self.lnprob)
        if verbose:
    ...
    def setup(self, reset=True, **kwargs):
        ...
        **kwargs could be any of the RUN_PROPERTIES (nrun, nwalkers, guess,
        guess_err)
        """
        ...
        for k, v in kwargs.items():
            ...
            if k in ["guess", "guess_err"] and len(v) != self.nparam:
                raise ValueError(
                    "'%s' must be a %d-value array (%d given)" % (k,
                                                                  self.nparam,
                                                                  len(v)))
            self.properties[k] = v
    ...
    def set_burnin(self, value):
        ...
        if value < 0 or value > self.nrun:
            raise ValueError("the mcmc burnin must be greater than 0" +
                             " and lower than the amount of run.")
    ...
    def load_data(self, mcmcdata):
        ...
        ----------
        mcmcdata: [dict]
            Dictionary as create by the MCMC class' method 'data', containing
            the keys:
        ...
        if type(mcmcdata) is not dict:
            raise TypeError(
                "mcmcdata must be a dict as created by the data method")
    ...
    def set_chain(self, chain):
        ...
        chain: [3D N-length array]
            chain containing the walkers information.
        ...
        if nfreeparameters != self.nparam:
            raise TypeError(
                "The chain do not have the good number of freeparameter")
    ...
    def show_corner(self, savefile=None, show=True,
        ...
        **kwargs goes to corner.corner
        ...
        try:
            ...
            raise ImportError(
                "install corner to be able to do this plot => " +
        ...
        fig = corner.corner(self.samples, labels=self.freeparameters,
                            ...
                            show_titles=True, label_kwargs={"fontsize":
                                                            "xx-large"},
                            **kwargs)
    ...
    def nwalkers(self, value):
        ...
        if int(value) < 2*self.nparam:
            raise ValueError(
                "emcee request to have at least twice more " +
                "walkers than parameters.")
    ...
    def draw_poswalkers(self):
        ...
        if self._boundaries_poswalkers is None:
            return [np.random.uniform(self.guess-self.guess_err,
                                      self.guess+self.guess_err,
                                      size=self.nparam)
                    for i in range(self.nwalkers)]

        bounds = np.asarray(self._boundaries_poswalkers, dtype="float")
        return [np.random.uniform(np.nanmax([self.guess-self.guess_err,
                                             bounds.T[0]], axis=0),
                                  np.nanmin(
                                      [self.guess+self.guess_err, bounds.T[1]],
                                      axis=0),
                                  size=self.nparam)
                for i in range(self.nwalkers)]
    ...
    def samples(self):
        ...
        if self.burnin is None:
            raise AttributeError(
                "You did not specified the burnin value. see 'set_burnin")
...
class _KFolder_(BaseObject):
    ...
    def fold_data(self, kfold, nsamples=1000):
        ...
        if int(kfold) < 2:
            raise ValueError(
                "kfold must be greater than 2 (int), %d given" % (kfold))
        nsamples = int(nsamples)
        if int(nsamples) < 1:
            raise ValueError(
                "nsamples must be at least 1 (int), %d given" % (nsamples))
        ...
        for i in range(nsamples):
            ...
            self._derived_properties["fold_indexes"].append(
                indexes[noutfold:].copy())
    ...
    def _foldindexes(self):
        ...
        if self._derived_properties["fold_indexes"] is None:
            warnings.warn(
                "No foilding index defined. returns the list of all indexes")
            self._derived_properties["fold_indexes"] = [
...
class DataSourceHandler(_KFolder_):
    ...
    def get(self, key, names=None, default=None):
        ...
        if is_arraylike(names):
            return np.asarray([self.data[name_][key]
                               if key in self.data[name_].keys()
                               else default
                               for name_ in names])

        return self.data[names][key] if key in self.data[names].keys() \
            else default
...
class BaseFitter(BaseObject):
    ...
    def fit(self, use_minuit=None, kfold=None, nsamples=1000,
        ...
        use_minuit: [bool/None]
            ...
            *self.use_minuit* value.
            ...
            The internal functions are made such that none of the other
            function the user have access to depend on the choice of fitter
            technique. For instance the fixing value concept (see set_guesses)
            remains with scipy.
        ...
        **kwargs parameter associated values with the shape:
            'parametername'_guess, 'parametername'_fixed,
            'parametername'_boundaries
        ...
        if kfold is not None and DataHandler not in self.__class__.__mro__:
            raise ValueError(
                "Only Fitter inherating from DataHandler can use k-folding. " +
                "Set kfold to None")
    ...
    def get_residuals(self, parameters=None):
        ...
        Array
        """
        return self.data - self.get_model(self._fitparams
    ...
    def set_model(self, model, **kwargs):
        ...
        **kwargs goes to self.set_guesses.
        ...
        try:
            ...
            raise ValueError(
                " You must define a 'get_modelchi2()' method in your fitter.")
        self.setup_guesses(**kwargs)
    ...
    def get_fitvalues(self, mcmc=False, nonsymerrors=False):
        ...
        Return
        ------
        dictionary (paramname=best_value ;
                    paramname.err = err or [errlow,errup])
        """
        ...
        **kwargs the v_guess, v_boundaries and, v_fixed for as many
        ...
        for k, v in kwargs.items():
            if "_guess" in k:
                ...
                raise ValueError(
                    "I am not able to parse %s ; " +
                    "not _guess, _fixed nor _boundaries" % k)
    ...
    def setup_mcmc(self, nrun=2000, walkers_per_dof=4,
                   init=None, init_err=None, verbose=True):
        """ Setup the basic property for the mcmc to run,
            you just need to say self.mcmc.run()
            to run it. See also self.run_mcmc()
        """
        self._derived_properties["mcmc"] = \
            MCMC(self.model.lnprob,
                 self.model.freeparameters,
                 boundaries_poswalkers=self._mcmc_initbounds)

        # -------------
        # - Set it up
        guess = np.asarray([self.fitvalues[name]
                            for name in self.model.freeparameters]) \
    ...
    def set_mcmc(self, mcmcdata):
        ...
        if mcmc.freeparameters != self.model.freeparameters:
            raise ValueError(
                "the mcmcdata freeparameters do not correspond " +
    ...
    def show_mcmcwalkers(self, savefile=None, show=True,
        ...
        self.mcmc.show_walkers(savefile=savefile, show=show,
                               cwalker=cwalker, cline=cline, truths=truths,
                               **kwargs)
    ...
    def _mcmc_initbounds(self):
        """ The mcmc init bounds for a faster mcmc convergence.
        Defautl parambounds """
        return self.parambounds if "_mcmc_initbounds" not in dir(self.model) \
            else self.model._mcmc_initbounds
    ...
    def mcmc_samples(self):
        """ the flatten samplers after burned in removal,
        see set_mcmc_samples """
    ...
    def mcmc_bestparam(self):
        """ best parameter best on the mcmc run.
        see mcmc_fitvalues for associated errors """
        if not self.has_mcmc():
            raise AttributeError("run mcmc first.")
    ...
    def covmatrix(self):
        ...
        if self.use_minuit:
            ...
            if "hess_inv" in self.scipy_output:
                ...
                fakeHess = np.zeros(
                    (len(self.scipy_output["x"]), len(self.scipy_output["x"])))
    ...
    def _fit_readout_(self, kfolding=False):
        """Gather the output in the readout,
        you could improve that in you child class"""

        for i, name in enumerate(self.model.freeparameters):
            self.fitvalues.setdefault(name, []).append(self._fitparams[i])
            self.fitvalues.setdefault(
                name+".err", []).append(np.sqrt(self.covmatrix[i, i]))
    ...
    def _fit_scipy_(self):
        ...
        self.scipy_output = minimize(self.model._scipy_chi2_,
                                     self._paramguess_scipy,
                                     bounds=self._parambounds_scipy)

        self._fitparams = self.model._read_scipy_parameter_(
            self.scipy_output["x"])
        self.fitOk = True
    ...
    def _setup_scipy_(self):
        ...
        self._paramguess_scipy, self._parambounds_scipy = \
            self.model._parameter2scipyparameter_(
                self.paramguess, self.parambounds)
...
class BaseModel(BaseObject):
    ...
    def get_model(self, parameters):
        ...
        raise NotImplementedError(
            "The Model has no get_model() defined. Do so.")
    ...
    def lnprior(self, parameters, verbose=True):
        ...
        if verbose:
            print(
                "Perfectly flat prior used. " +
                "Always 0 (set verbose=False to avoid this message)")
        return 0
    ...
    def freeparameters(self):
        ...
        You could set FREEPARAMETERS in the global argument of a class
        to uses this.
        """
    ...
    def set_param_input(self, param_input):
        ...
        for k in param_input.keys():
            if "_guess" not in k and \
                    "_fixed" not in k and \
                    "_boundaries" not in k:
                raise ValueError(
                    "Parameters input must have information like " +
                    "_guess, _fixed, _boundaries only")
    ...
    def _read_hess_(self, hess):
        ...
        indexFixed = [i for i, name in enumerate(self.freeparameters)
                      if "%s_fixed" % name in dir(self) and
                      eval("self.%s_fixed" % name)]
        for i in indexFixed:
